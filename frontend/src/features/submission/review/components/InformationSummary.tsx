    ...
    <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 space-y-4 shadow-sm text-left relative overflow-hidden font-sans">
      ...
      {information ? (
        <div className="space-y-3.5 text-xs text-slate-700">
          <div>
            <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block mb-0.5">{t('reviewTitleLabel')}</span>
            <p className="text-slate-900 font-extrabold leading-snug">{information.title}</p>
          </div>
          <div>
            <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block mb-0.5">{t('reviewDescLabel')}</span>
            <p className="text-slate-600 leading-relaxed whitespace-pre-line font-medium">{information.description}</p>
          </div>
          <div className="grid grid-cols-2 gap-4 pt-1">
            <div>
              <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block mb-0.5">{t('reviewCategoryLabel')}</span>
              <p className="text-slate-900 font-extrabold">{t(information.category as any) || information.category}</p>
            </div>
            <div>
              <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block mb-0.5">{t('reviewLanguageLabel')}</span>
